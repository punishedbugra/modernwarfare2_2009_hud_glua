...
local cvar_enabled = CreateClientConVar("mw2_suspense_enabled", "1", true, false, "Enable or disable MW2 suspense music")
...
cvars.AddChangeCallback("mw2_suspense_enabled", function(name, old, new)
    if tobool(new) then
        ScheduleNextSuspense()
    else
        if currentSoundObj then
