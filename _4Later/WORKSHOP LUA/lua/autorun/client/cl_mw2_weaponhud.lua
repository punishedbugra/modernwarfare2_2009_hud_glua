...
local function DrawSqueezedText(text, font, x, y, color, squeeze, squeezeOne, align, squeezeOneBefore)
    local str = language.GetPhrase(tostring(text))
...
hook.Add("HUDPaint", "MW2_MergedHUD", function()
    ...
    if clip >= 0 and maxClip > 0 then
        ...
        if statText ~= "" then
            ...
            local str = language.GetPhrase(tostring(statText))
            draw.SimpleText(str, "MW2_Stat_Font", cx + SX(2), cy + SY(2), Color(0, 0, 0, finalCol.a * 0.8), 1, 0)
            draw.SimpleText(str, "MW2_Stat_Font", cx, cy, finalCol, 1, 0)
        end
    end
end)