...
local MW2_RS_FACTIONS = {
    rangers      = { short = "US", header = "Army Rangers",   icon = "factions/faction_128_rangers.png",      glow = Color(240, 230, 190) },
    taskforce141 = { short = "UK", header = "Task Force 141", icon = "factions/faction_128_taskforce141.png", glow = Color(225, 225, 255) },
    seals        = { short = "NS", header = "Navy SEALs",     icon = "factions/faction_128_seals.png",        glow = Color(170, 170, 185) },
    ussr         = { short = "RU", header = "Spetsnaz",       icon = "factions/faction_128_ussr.png",         glow = Color(185, 140, 120) },
    arab         = { short = "AB", header = "OpFor",          icon = "factions/faction_128_arab.png",         glow = Color(220, 180, 150) },
    militia      = { short = "PG", header = "Militia",        icon = "factions/faction_128_militia.png",      glow = Color(150, 85, 85)   },
}

local MW2_RS_OBJECTIVES = {
    ["Team Deathmatch"]    = "Eliminate enemy players.",
    ["Free-for-All"]       = "Eliminate other players.",
    ["Domination"]         = "Capture the flags and defend them.",
    ["Search and Destroy"] = "Destroy all objectives.",
}
...
local MW2_RS_BOOST_VO = {
    US = "announcer/US/US_1mc_boost_01.mp3",
    UK = "announcer/UK/UK_1mc_boost_01.mp3",
    NS = "announcer/NS/NS_1mc_boost_01.mp3",
    RU = "announcer/RU/RU_1mc_boost_01.mp3",
    AB = "announcer/AB/AB_1mc_boost_01.mp3",
    PG = "announcer/PG/PG_1mc_boost_01.mp3",
}
...
local function MW2_RS_Start(gamemode)
    ...
    if not MW2_RS_FACTIONS[fkey] then fkey = "rangers" end
    local fdata = MW2_RS_FACTIONS[fkey]
    ...
    rs_short      = fdata.short
    rs_glow       = fdata.glow
    rs_icon_mat   = Material(fdata.icon, "smooth")
    rs_icon_alpha = 0

    rs_h_text    = fdata.header
    rs_h_written = 0
    rs_h_nxt_w   = CurTime()
    rs_h_done    = false
    rs_h_blanks  = {}
    ...
    rs_o_text    = MW2_RS_OBJECTIVES[gamemode] or "Eliminate enemy players."
    rs_o_written = 0
    rs_o_done    = false
    rs_o_erasing = false
    rs_o_blanks  = {}
...
hook.Add("Think", "MW2_RS_Think", function()
    ...
    elseif rs_phase == "erase_faction" then
        ...
        if not rs_boost_done then
            rs_boost_done = true
            if MW2_RS_BOOST_VO[rs_short] then
                surface.PlaySound(MW2_RS_BOOST_VO[rs_short])
            end
        end
...
hook.Add("HUDPaint", "MW2_RS_Draw", function()
    ...
    if rs_phase == "faction" then
        ...
        if disp > 0 then
            ...
            draw.SimpleText("Match begins in:", "MW2_RS_S_Pri", tx, ty + syo, Color(255, 255, 255, 200), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
    end
...
local function MW2_RS_OpenConfirm()
    ...
    rs_confirm:Center()
    rs_confirm:SetTitle("Round Start")
    rs_confirm:MakePopup()

    local lbl = vgui.Create("DLabel", rs_confirm)
    lbl:SetPos(10, 30)
    lbl:SetSize(400, 60)
    lbl:SetText("Are you sure you want to reset all scores and start the match?")
    lbl:SetWrap(true)
    lbl:SetDark(true)
...
hook.Add("PopulateToolMenu", "MW2_RS_PopMenu", function()
    spawnmenu.AddToolMenuOption("Options", "MW2", "MW2_RoundStart", "#Round Start", "", "", function(panel)
        panel:ClearControls()
        ...
        start_btn:SetText("Start Round (Admin Only)")
